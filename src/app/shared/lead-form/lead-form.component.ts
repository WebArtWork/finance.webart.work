...
export type LeadFormKind = 'user' | 'advisor' | 'company' | 'investor';

const KIND_LABELS: Record<LeadFormKind, string> = {
	user: 'Заявка користувача',
	advisor: 'Заявка радника',
	company: 'Заявка компанії',
	investor: 'Заявка інвестора',
};

/** Inline lead-capture form used on the /for-* landing pages. Sends to the Finance Telegram contact channel. */
@Component({
...
export class LeadFormComponent {
	...
	protected async submit(form: NgForm): Promise<void> {
		...
		const lines = [
			`Finance lead: ${KIND_LABELS[this.kind()]}`,
			`Ім'я: ${this.name()}`,
